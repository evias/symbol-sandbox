...
export default class extends BaseCommand {
    ...
    async execute(options: CommandOptions) {
        ...
        let mosaicId1,
            ...
            supplyMod2;
        try {
            mosaicId1 = OptionsResolver(options,
                'mosaicId1',
                () => { return ''; },
                'Enter a mosaicId (array notation or hexadecimal): ');

            if (mosaicId1.indexOf('[') === 0) {
                // from array notation
                mosaicId1 = new MosaicId(JSON.parse(mosaicId1))
            }
            else {
                // from hex
                mosaicId1 = new MosaicId(RawUInt64.fromHex(mosaicId1))
            }

            mosaicId2 = OptionsResolver(options,
                'mosaicId2',
                () => { return ''; },
                'Enter a mosaicId (array notation or hexadecimal): ');

            if (mosaicId2.indexOf('[') === 0) {
                // from array notation
                mosaicId2 = new MosaicId(JSON.parse(mosaicId2))
            }
            else {
                // from hex
                mosaicId2 = new MosaicId(RawUInt64.fromHex(mosaicId2))
            }
        ...
        return await this.addSupplyForMosaic(mosaicId1, changeAction1, supplyMod1, mosaicId2, changeAction2, supplyMod2);
    }
